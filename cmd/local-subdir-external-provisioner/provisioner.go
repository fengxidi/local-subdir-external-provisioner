...
func (p *localProvisioner) Delete(ctx context.Context, volume *v1.PersistentVolume) error {
	path := volume.Spec.PersistentVolumeSource.Local.Path
...
func main() {
	...
	if nodeName == "" {
		glog.Fatalf("environment variable nodeName is not set! Please set it.")
	}

	path := os.Getenv("LOCAL_PATH")
	if path == "" {
		glog.Fatal("environment variable LOCAL_PATH is not set! Please set it.")
	}
	...
	clientLocalProvisioner := &localProvisioner{
		client:   clientset,
		nodeName: nodeName,
		path:     path,
	}
